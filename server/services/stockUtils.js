...
const generateStockData = async (symbol) => {
  const stocks = await Stock.findOne({ symbol });
  if (!stocks) {
  ...
  const lastItem = stocks.dayTimeSeries[stocks.dayTimeSeries.length - 1];

  if (!lastItem || now - new Date(lastItem.timestamp) > 1 * 60 * 1000) {
    stocks.dayTimeSeries.push({
      timestamp,
      time,
    ...
    const updateLow = Math.max(lastItem.low, close - Math.random() * 1);
    const updateCandle = {
    ...
    stocks.dayTimeSeries[stocks.dayTimeSeries.length - 1] = updateCandle;
  }
  stocks.dayTimeSeries = stocks.dayTimeSeries.slice(-390);
  stocks.currentPrice = close;
  try {
    await stocks.save();
  } catch (error) {
